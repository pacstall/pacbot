...
use libpacbot::github::get_github_token;
...
pub async fn manage_issues_for_outdated_pacscripts(client: &Client) -> PacResult {
    ...
    let response = client
        ...
        .run_graphql(ListIssuesQuery::build(ListIssuesQueryVariables {
            states: Some(vec![IssueState::Open]),
        }))
        .await?;

    let open_issues: Vec<_> = if let Some(data) = response.data {
        data.repository
            .expect("No repository found")
            .issues
            .nodes
            .expect("No issue nodes")
            .into_iter()
            .map(|node| node.expect("Unable to unwrap node"))
            .collect()
    } else {
        return Err(format!("{:#?}", response.errors.unwrap()).into());
    };
    ...
    //         if closed_issue.state_reason.as_ref().unwrap() ==
    ...
    //                 let title = format!("Outdated `{name}`: `{version}` ->
    // `{latest_version}`");
    //
    //                 // Only create issue if one with the same title doesn't
    // already exist                 if !github_open_issues.iter().any(|issue|
    // issue.title == title)                     && !github_closed_issues
    //                         .iter()
    //                         .any(|issue| issue.title == title)
    //                 {
    //                     let maintainer = package.maintainer;
    //                     let maintainer_split: Vec<&str> =
    // maintainer.split('<').collect();
    //
    //                     let maintainer_name =
    //                         if maintainer_split.len() != 2 &&
    // !maintainer.is_empty() {                             // There's no email
    // address present, so the whole string is the
    // // maintainer's name
    // Some(maintainer.trim().to_lowercase())                         } else if
    // !maintainer_split[0].is_empty() {                             // Use the
    // first element from the split, i.e, the maintainer's name
    // Some(maintainer_split[0].trim().to_lowercase())                         }
    // else {                             // There's no maintainer
    //                             None
    //                         };
    //
    ...
    //
    //                                 > **Warning**
    //                                 > This should not be closed manually, or by
    //                                 > using the *fixes*/*closes* keywords in your
    //                                 > PR. This bot will take care of that for
    //                                 > you."
    //                             );
    //
    ...
    //                             let body = raw_body
    //                                 .lines()
    //                                 .map(|line| line.trim_matches(|c| c == '
    // ').to_owned() + "\n")
    // .collect::<String>();
    //
    //                             // Check if the maintainer's name is a valid
    // username on github                             if client
    ...
    //                             } else {
    //                                 tracing::warn!(
    //                                     "{username} is not a valid GitHub
    // username, so not \                                      assigning them"
    //                                 );
    //                                 json!({
    //                                     "title": title,
    //                                     "body": body,
    ...
    //
    //                         // Close all the older issues associated with that
    // pacscript.                         for issue in
    // github_open_issues.iter().filter(|issue| {                             //
    // Find the first ` character                             let start_index =
    // issue.title.find('`').unwrap();
    //
    //                             // Find the index of the second ` character,
    // starting from the index of the                             // first `
    // character                             let end_index =
    // issue.title[start_index + 1..].find('`').unwrap();
    //
    //                             // Extract the string between the two `
    // characters, this is the package                             // name of
    // the issue                             let substring =
    // &issue.title[(start_index + 1)..=(start_index + end_index)];
    //
    //                             substring == name && issue.number !=
    // response.number                         }) {
    ...
    // {github_token}"))                             .body(json!({"body":
    // format!("Superseded by {}", response.html_url)}).to_string())
    // .send()                             .await?
    //                             .error_for_status()
    //                             .is_ok() {
    //                                 tracing::debug!("Commented on #{} about the
    // new issue #{}", issue.number, response.number);
    // } else {                                 tracing::error!("Unable to
    // comment on #{} about the new issue #{}", issue.number, response.number);
    //                             }
    //
    //                             // Close the issue
    //                             if client
    ...
    //                             {
    //                                 tracing::info!(
    //                                     "Closed issue number: {} as it's
    // superseded by {}",                                     issue.number,
    //                                     response.number,
    //                                     );
    //                             } else {
    //                                 tracing::error!("Failed to close issue
    // number: {}", issue.number);                             }
    //                         }
    //                     } else {
    //                         tracing::warn!(
    //                             "{name}'s maintainer is not in our organization,
    // so assigning them \                             failed"
    //                         );
    //                     }
    //                 }
    //             }
    ...
    //
    //                     // Find the index of the second ` character, starting
    // from the index of the                     // first ` character
    //                     let end_index = issue.title[start_index +
    // 1..].find('`').unwrap();
    //
    //                     // Extract the string between the two ` characters, this
    // is the package                     // name of the issue
    //                     let substring = &issue.title[(start_index +
    ...
    //                         .header("accept", "application/vnd.github+json")
    //                         .header("Authorization", format!("token
    // {github_token}"))                         .body(json!({"state":
    // "close"}).to_string())                         .send()
    //                         .await?
    //                         .error_for_status()
    //                         .is_ok()
    //                     {
    //                         tracing::info!(
    //                             "Closed issue number: {} as {name} is
    // up-to-date",                             issue.number
    //                         );
    //                     } else {
    //                         tracing::error!("Failed to close issue number: {}",
    // issue.number);                     }
    //                 }
    //             }
    //         }
